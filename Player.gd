...
#    input.x = Input.get_action_strength("ui_right") - Input.get_action_strength("ui_left")
    input.x = Input.get_axis("ui_left", "ui_right") # cleaner shortcut of above
    input.y = Input.get_axis("ui_up", "ui_down") # cleaner shortcut of above
...
func move_state(input):
    ...
    # Coyote jump takes priority over double jump. Else might be a bug?
    if can_jump():
        input_jump()
    else:
        # input jump release
        ...
        # Jump Buffer logic. Player just pressed right before hitting the ground -- buffer a jump
        buffer_jump()
...
func reset_double_jump():
    # Reset double jump
    double_jump = moveData.DOUBLE_JUMP_COUNT # Reset double_jump
